...
class CloudShellInventoryUtilities:
    ...
    def create_n_autoload(self):
        ...
        for ro in range(5, sheet.nrows):
            ...
            if row.valid and not row.ignore:
                print ' - Working on {}'.format(row.name)
                try:
                    ...
                    # preform autoload
                    if row.autoload:
                        print ' -- Running Autoload on {} (this may take a minute)'.format(row.name)
    ...
    def set_attributes(self):
        ...
        for ro in range(5, sheet.nrows):
            ...
            if not row.ignore:
                print ' - Setting Attributes for {}'.format(row.name)
                for att in custom_attributes:  # walk the headers and assign if they match (skip blanks)
                    if str(sheet.cell(ro, custom_attributes.index(att))).strip() != '':  # not empty
                        full_att_name = self.has_attribute(att, self.attribute_names(row.name))
                        if full_att_name != BAD_VALUE:
                            self.set_attribute_value(device_name=row.name, attribute_name=full_att_name,
    ...
    def list_connections(self):
        ...
        csv_filepath = '{}/reports/current_connections_{}.csv'.format(os.getcwd(), strftime('%Y_%m_%d_%Hh_%Mm'))
        ...
        try:
            for item in device_query_list:
                ...
                self.connection_list = []
                print ' - Gathering Connections for {}'.format(item)
    ...
    def set_connections(self):
        ...
        for ro in range(5, sheet.nrows):
            row = row_helpers.SetConnectionsRow(sheet.row(ro))
            if not row.ignore:
                print ' - Setting Connection: {} {}'.format(row.point_a, row.point_b)
                if row.point_a:
                    if row.point_b:
                        self._make_connection(row.point_a, row.point_b)
                    else:
    ...
    def generate_user_report(self):
        ...
        csv_filepath = '{}/reports/user_report_{}.csv'.format(os.getcwd(), strftime('%Y_%m_%d_%Hh_%Mm'))
    ...
    def generate_inventory_report(self):
        ...
        csv_filepath = '{}/reports/inventory_report_{}.csv'.format(os.getcwd(), strftime('%Y_%m_%d_%Hh_%Mm'))
